...
class VanillaSponge extends Sponge
{
    ...
    {
        parent::__construct(new BlockIdentifier(BlockTypeIds::SPONGE), 'Sponge', new BlockTypeInfo(new  BlockBreakInfo(0.6, BlockToolType::HOE)));
    }
    ...
    private function absorb() : void {
        ...
        for ($x = 0; $x <= 16; $x++) {
            for ($y = 0; $y <= 16; $y++) {
                for ($z = 0; $z <= 16; $z++) {
                    $id = $world->getBlockAt($startX + $x, $startY + $y, $startZ + $z)->getTypeId();
                    if ($id === BlockTypeIds::WATER || $id === BlockTypeIds::WATER_CAULDRON || $id === BlockTypeIds::LAVA || $id === BlockTypeIds::LAVA_CAULDRON) {
                        $world->setBlockAt($startX + $x, $startY + $y, $startZ + $z, VanillaBlocks::AIR());
                        $absorbed = true;
                    }
                }
            }
